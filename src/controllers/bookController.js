...
const findBooks= async function(req,res)
{
    ...
    res.send({msg:data,all})
}
module.exports.findBooks=findBooks

const findAuthor= async function (req,res){
    
    let All=await authorModel.findOne({ Name:"Two states"}).select({author_Name:1,_id:0})
    let list=await BookModel.findOneAndUpdate({author_Name:All.authorName},{$set:{price:150}},{new:true})
